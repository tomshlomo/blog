...
linear in $t$ once again, with only the per-factor rates changed from the triangle case @eq-tri-Dt, from the preconditioned nuclear norms to the ratios $\|G_Q\|_*/\|W_K\|_2$ and $\|G_K\|_*/\|W_Q\|_2$. So the same budget-split argument applies,[^triangle-opt] and the optimum is again **winner-take-all**: the whole budget on whichever of $W_Q$, $W_K$ has the larger ratio. As in the triangle case, this gives two variants, **winner-take-all** and **symmetric cross-scaled Muon**, where the symmetric $t = \tfrac12$ choice updates both factors with an ordinary Muon step, each damped by the spectral size of its partner.
...
The most obvious next step is to actually run these optimizers and measure how they perform. The analysis here says nothing about which variant wins in practice, or whether the winner-take-all split really does hurt as much as I suspect. While writing this, I came across a recent [blog post from Tilde Research](https://blog.tilderesearch.com/blog/compositional-muon) that explores very similar ideas and goes further: they implement the approach and report it setting a new state-of-the-art on the nanochat speedrun. Well worth a read.
...
A third direction is regularization. Decoupled weight decay (AdamW) is a near-universal improvement over plain Adam, which motivates adding a regularization term to the steepest-descent problems above. That turns each update into a proximal step, a shrinkage operation, but now one that acts on the bilinear form itself (say, spectral or nuclear shrinkage of $W_{QK}$) rather than on the raw factor parameters, which is arguably the more natural object to penalize.